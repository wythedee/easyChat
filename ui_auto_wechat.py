...
class WeChat:
    def __init__(self, path):
        ...
        self.zhipuai_key = "your_zhipuai_key"
    ...
    def set_zhipuai_key(self, zhipuai_key):
        self.zhipuai_key = zhipuai_key
    ...
    def check_new_msg(self):
        ...
        while True:
            ...
            current_name = item.ButtonControl().Name
            if len(temp) == 3:
                print(f"{current_name} 有新消息")
                
                # 判断该联系人是否需要自动回复
                if current_name in self.auto_reply_contacts:
                    dialogs = self.get_dialogs(current_name, 1)[0]
                    resp = ""
                    if dialogs[0] == "撤回消息" or dialogs[2].startswith( '[' ):
                        ...
                        self._auto_reply(item, resp)
                    elif dialogs[0] == prev_name or dialogs[2].startswith( '@' ):
                        print("私聊或者被人@了，调用chatbot回复")
                        resp = self.get_result_from_LLMs("zhipu", dialogs[2])["msg"]
                        self._auto_reply(item, "[chatbot]"+resp)
                    else:
                        print(f"调用了LLM:{self.LLM_name}来认真的敷衍")
                        resp = self.get_result_from_LLMs(self.LLM_name, dialogs[2])["msg"]
                        self._auto_reply(item, resp)
                    print(f"自动回复 {current_name} : {resp}")
                    time.sleep(5)
    ...
    def get_result_from_LLMs(self, LLM_name, str) -> dict:
        if LLM_name == "qianfan":
            return self.get_result_qianfan(str= str)
        elif LLM_name == "zhipu":
            return self.get_result_zhipuai(str= str)
        


    #从qianfan大模型中获取回复，用于自动回复
    def get_result_qianfan(self, str) -> dict:
        ...
        try:
            ...
            print("json解析失败或者请求失败了，回复颜文字")
            result = {"msg": "Σ(°ロ°)"}
        return result

    def get_result_zhipuai(self, str) -> dict:
        zhipuai.api_key = self.zhipuai_key
        prompt = [{
            "role": "user",
            "content": "你是一个日常的聊天助手，需要尽可能的解答或回复以下消息，\
                消息示例如下：\
                你是谁"
        },
        {
